...
public:
	...
	{
		return ((y > 0)?-1:1)*Math.acos(x)*180/V2D_PI;
	}
	...
	V2D<VTYPE> operator+(const V2D<VTYPE> & rhs){ return sum(rhs); }
	V2D<VTYPE> operator-(const V2D<VTYPE> & rhs){ return difference(rhs); }
...
// OpenGL/GLUT specific functions
	...
	void glDrawString(char * a_string) const
	{
		glRasterPos2f (x, y);
		unsigned int i;
